...
def ask(provider: str, prompt: str, model: str | None, timeout: int) -> str:
    if provider == "codex":
        ...
        command = ["claude", "-p", "--bare"] + (["--model", model] if model else [])
...
def run(provider: str, model: str, episodes: list[tuple[int, int]], use_memory: bool, timeout: int) -> list[dict[str, object]]:
...
def main() -> None:
    ...
    results = run(args.provider, model, episodes, args.memory, args.timeout)
    payload = {"provider": args.provider, "model": model or "default", "memory": args.memory, "episodes": results}
